...
headers.forEach(header => {
    ...
    header.addEventListener('click', (event) => {
        ...
        if (isSpecial) {
            ...
            // Guardar estado inicial de la sección actual
            const categoryList = section.querySelector('.category-list');
            const wasCollapsed = categoryList.classList.contains('collapsed');

            // Cerrar todas las secciones abiertas excepto la actual
            headers.forEach(otherHeader => {
                const otherSection = otherHeader.parentElement;
                const otherCategoryList = otherSection.querySelector('.category-list');
                const otherExpandIndicator = otherHeader.querySelector('.expand-indicator');

                if (otherSection !== section && !otherCategoryList.classList.contains('collapsed')) {
                    otherCategoryList.classList.add('collapsed');
                    if (otherExpandIndicator) otherExpandIndicator.innerHTML = '▼';
                }
            });
